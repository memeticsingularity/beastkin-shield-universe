# 长篇分章故事 / Chaptered Stories

> 返回上级：[改编作品根](../README.md)
> 返回世界观：[世界观根](../../../README.md)

---

## 一、形式说明 / Format Overview

长篇分章故事采用多章节连载结构，每章独立成篇又相互衔接。改编作品在保留原作核心设定的基础上，探索退役兵、新兵等不同视角的叙事可能性。

---

## I. Format Overview

Chaptered stories adopt a multi-chapter serialized structure, where each chapter stands alone while connecting to the larger narrative. Adaptation works preserve core original settings while exploring narrative possibilities from different perspectives such as retired soldiers and new recruits.

---

## 二、作品列表 / Works List

| 编码 | 中文标题 | 英文标题 | 状态 | 标签 | 链接 |
|------|---------|---------|------|------|------|
| `eks-o-cm-1-recall-protocol` | 召回协议 | Recall Protocol | 已完成 | 原创,退役,虎兽人 | [目录](eks-o-cm-1-recall-protocol/README.md) |
| `eks-o-cm-2-dog-soldier-dreams` | 犬兵梦忆 | Dog Soldier Dreams | 已完成 | 原创,退役,犬兽人 | [目录](eks-o-cm-2-dog-soldier-dreams/README.md) |

---

## II. Works List

| Code | Title | Status | Tags | Link |
|------|-------|--------|------|------|
| `eks-o-cm-1-recall-protocol` | Recall Protocol | Completed | original,retired,tiger | [Directory](eks-o-cm-1-recall-protocol/README.md) |
| `eks-o-cm-2-dog-soldier-dreams` | Dog Soldier Dreams | Completed | original,retired,dog | [Directory](eks-o-cm-2-dog-soldier-dreams/README.md) |

---

## 三、创作规范速查 / Quick Reference

- 编码规范：[docs/spec/02-work-coding.md](../../../../../../docs/spec/02-work-coding.md)
- 故事格式：[docs/spec/11-story-format.md](../../../../../../docs/spec/11-story-format.md)
- 内容指南：[docs/spec/12-content-guidelines.md](../../../../../../docs/spec/12-content-guidelines.md)

---

## III. Quick Reference

- Coding spec: [docs/spec/02-work-coding.md](../../../../../../docs/spec/02-work-coding.md)
- Story format: [docs/spec/11-story-format.md](../../../../../../docs/spec/11-story-format.md)
- Content guidelines: [docs/spec/12-content-guidelines.md](../../../../../../docs/spec/12-content-guidelines.md)

---

*最后更新：2026-05-16 · Last updated: 2026-05-16*
