# 短篇故事集 / Short Stories Collection

> 收录兽盾宇宙独立短篇故事，每篇故事自成一体，可按任意顺序阅读。
> A collection of standalone short stories set in the BeastShield Universe. Each story is self-contained and can be read in any order.
>
> 各故事均包含成人内容（暴力、性、死亡等），阅读前请注意分级标识。
> All stories contain adult content (violence, sexual themes, death, etc.). Please note the age rating before reading.

---

## 一、故事列表（按编号排序）/ Story List (Sorted by Number)

| 编码 | 中文标题 | 英文标题 | 标签 | 链接 |
|------|----------|----------|------|------|
| `bs-o-s-1-farm-inn` | 兽盾农家乐 | Beast Shield Farmhouse | `悬疑` `黑暗` `农场` `实验` `复仇` | [阅读](bs-o-s-1-farm-inn/bs-o-s-1-farm-inn.md) |
| `bs-o-s-2-blackstone` | 黑石 | Blackstone | `动作` `惊悚` `赏金猎人` `潜入` | [阅读](bs-o-s-2-blackstone/bs-o-s-2-blackstone.md) |
| `bs-o-s-3-blackstone-2` | 黑石2 | Blackstone 2 | `动作` `惊悚` `夜袭` `狩猎` | [阅读](bs-o-s-3-blackstone-2/bs-o-s-3-blackstone-2.md) |
| `bs-o-s-4-captive-delivery` | 俘虏快递 | Captive Delivery | `心理` `惊悚` `俘虏` `道德困境` | [阅读](bs-o-s-4-captive-delivery/bs-o-s-4-captive-delivery.md) |
| `bs-o-s-5-employee-diary` | 兽盾员工的一天 | Beast Shield Employee's Day | `日记` `惊悚` `生存` `屠杀` | [阅读](bs-o-s-5-employee-diary/bs-o-s-5-employee-diary.md) |
| `bs-o-s-6-night-raid` | 夜袭 | Night Raid | `惊悚` `潜入` `夜袭` `羞辱处决` | [阅读](bs-o-s-6-night-raid/bs-o-s-6-night-raid.md) |
| `bs-o-s-7-graduation-internship` | 毕业实习 | Graduation Internship | `实习` `成长` | [阅读](bs-o-s-7-graduation-internship/bs-o-s-7-graduation-internship.md) |
| `bs-o-s-8-the-night-watch` | 守夜人 | The Night Watch | `悲剧` `G级杂兵视角` `赏金猎人` `情感深度` | [目录](bs-o-s-8-the-night-watch/README.md) |

---

## 二、发现入口 / Discovery

### 按标签筛选 / Filter by Tags

- **按主题 / By Theme**:
    - `赏金猎人 / Bounty Hunter`: 黑石、黑石2、守夜人
    - `俘虏/控制 / Captive/Control`: 俘虏快递
    - `日记/生存 / Diary/Survival`: 兽盾员工的一天
    - `农场/实验 / Farm/Experiment`: 兽盾农家乐
    - `夜袭/潜入 / Night Raid/Infiltration`: 黑石2、夜袭、守夜人

- **按风格 / By Tone**:
    - `惊悚 / Thriller`: 黑石、黑石2、俘虏快递、兽盾员工的一天、夜袭、守夜人
    - `黑暗 / Dark`: 兽盾农家乐、俘虏快递
    - `动作 / Action`: 黑石、黑石2

### 按兽人种族 / By Beastkin Race

- 牛兽人 / Bovine: 兽盾农家乐
- 野猪兽人 / Wild Boar: 守夜人
- 熊兽人 / Bear: 兽盾员工的一天
- 虎兽人 / Tiger: 兽盾员工的一天（红虎）

---

## 三、阅读建议与分级说明 / Reading Advice & Content Warnings

- **R18**: 所有故事均包含成人内容，包括但不限于：
    - 暴力、勒杀、窒息、虐杀
    - 性内容、性暴力、非自愿行为
    - 死亡、尸体处理、道德困境
- **建议读者**: 仅适合18岁以上、对黑暗主题与成人内容有心理准备的读者。
- **阅读顺序**: 各篇独立，可随意跳读，但按编号顺序可渐入世界观。

---

## 四、语言版本 / Language Versions

- [中文版 / Chinese Version](./) (当前页面 / current)
- [英文版 / English Version](../../english/short-stories/README.md)

---

[返回上级 / Return Up](../README.md)

*最后更新 / Last Updated: 2026-05-16*
