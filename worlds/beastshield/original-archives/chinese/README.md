# 中文原作 / Chinese Originals

> 《兽盾》世界观官方中文原作存档。
> Official Chinese original archives of the BeastShield universe.

---

## 一、目录结构 / Directory Structure

| 路径 | 说明 | Description |
|------|------|-------------|
| [`chaptered-stories/`](chaptered-stories/) | 分章长篇 | Chaptered stories |
| [`short-stories/`](short-stories/) | 短篇故事 | Short stories |

---

## 二、分章长篇作品 / Chaptered Stories

### 主线 / Main

| 编码 | 标题 | 标签 | 链接 |
|------|------|------|------|
| `bs-o-cm-1-main-story-1` | 兽盾主线故事 第一卷·兽游世界 | `主线` `VR` `游戏机制` | [目录](chaptered-stories/main/bs-o-cm-1-main-story-1/README.md) |
| `bs-o-cm-1-main-story-2` | 兽盾主线故事 第二卷·魔影 | `主线` `特工局` `魔影` | [目录](chaptered-stories/main/bs-o-cm-1-main-story-2/README.md) |
| `bs-o-cm-1-main-story-3` | 兽盾主线故事 第三卷·白之都 | `主线` `最终决战` `十二神武` | [目录](chaptered-stories/main/bs-o-cm-1-main-story-3/README.md) |

### 支线 / Side

| 编码 | 标题 | 标签 | 链接 |
|------|------|------|------|
| `bs-o-cs-1-yan-liang` | 炎魉 | `支线` `炎魉` | [目录](chaptered-stories/side/bs-o-cs-1-yan-liang/README.md) |
| `bs-o-cs-2-mo-quan` | 墨犬 | `支线` `墨犬` `武侠` | [目录](chaptered-stories/side/bs-o-cs-2-mo-quan/README.md) |

---

## 三、短篇作品 / Short Stories

| 编码 | 标题 | 标签 | 链接 |
|------|------|------|------|
| `bs-o-s-1-farm-inn` | 兽盾农家乐 | `悬疑` `黑暗` `农场` | [阅读](short-stories/bs-o-s-1-farm-inn/bs-o-s-1-farm-inn.md) |
| `bs-o-s-2-blackstone` | 黑石 | `动作` `惊悚` `赏金猎人` | [阅读](short-stories/bs-o-s-2-blackstone/bs-o-s-2-blackstone.md) |
| `bs-o-s-3-blackstone-2` | 黑石2 | `动作` `惊悚` `夜袭` | [阅读](short-stories/bs-o-s-3-blackstone-2/bs-o-s-3-blackstone-2.md) |
| `bs-o-s-4-captive-delivery` | 俘虏快递 | `心理` `惊悚` `俘虏` | [阅读](short-stories/bs-o-s-4-captive-delivery/bs-o-s-4-captive-delivery.md) |
| `bs-o-s-5-employee-diary` | 兽盾员工的一天 | `日记` `惊悚` `生存` | [阅读](short-stories/bs-o-s-5-employee-diary/bs-o-s-5-employee-diary.md) |
| `bs-o-s-6-night-raid` | 夜袭 | `惊悚` `潜入` `夜袭` | [阅读](short-stories/bs-o-s-6-night-raid/bs-o-s-6-night-raid.md) |
| `bs-o-s-7-graduation-internship` | 毕业实习 | `实习` `成长` | [阅读](short-stories/bs-o-s-7-graduation-internship/bs-o-s-7-graduation-internship.md) |
| `bs-o-s-8-the-night-watch` | 守夜人 | `悲剧` `G级杂兵视角` `赏金猎人` | [目录](short-stories/bs-o-s-8-the-night-watch/README.md) |

---

## 四、发现入口 / Discovery

### 按标签筛选 / Filter by Tags

- `主线 / Main`: 兽游世界、魔影、白之都
- `支线 / Side`: 炎魉、墨犬
- `赏金猎人 / Bounty Hunter`: 黑石、黑石2、守夜人
- `动作 / Action`: 黑石、黑石2
- `惊悚 / Thriller`: 黑石、黑石2、俘虏快递、兽盾员工的一天、夜袭、守夜人
- `心理 / Psychological`: 俘虏快递
- `日记 / Diary`: 兽盾员工的一天
- `农场 / Farm`: 兽盾农家乐
- `夜袭 / Night Raid`: 黑石2、夜袭、守夜人
- `G级杂兵视角 / Grunt Perspective`: 守夜人

### 按兽人种族 / By Beastkin Race

- 牛兽人 / Bovine: 兽盾农家乐
- 野猪兽人 / Wild Boar: 守夜人
- 熊兽人 / Bear: 兽盾员工的一天
- 虎兽人 / Tiger: 兽盾员工的一天（红虎）

---

## 五、语言版本 / Language Versions

- [中文版 / Chinese Version](./) (当前页面 / current)
- [英文版 / English Version](../english/README.md)

---

[返回上级 / Return Up](../README.md)

*最后更新 / Last Updated: 2026-05-16*
