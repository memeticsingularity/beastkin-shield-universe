# English Originals / 英文原作

> Official English original archives of the BeastShield universe.
> 《兽盾》世界观官方英文原作存档。

---

## I. Directory Structure / 目录结构

| Path | Description | 说明 |
|------|-------------|------|
| [`chaptered-stories/`](chaptered-stories/) | Chaptered stories | 分章长篇 |
| [`short-stories/`](short-stories/) | Short stories | 短篇故事 |

---

## II. Chaptered Stories / 分章长篇

### Main / 主线

| Code | Title | Tags | Link |
|------|-------|------|------|
| `bs-o-cm-1-main-story-1` | Beastshield Main Story Vol.1: Beast Game World | `main` `VR` `game-mechanics` | [Index](chaptered-stories/main/bs-o-cm-1-main-story-1/README.md) |
| `bs-o-cm-1-main-story-2` | Beastshield Main Story Vol.2: Moying | `main` `agent-bureau` `moying` | [Index](chaptered-stories/main/bs-o-cm-1-main-story-2/README.md) |
| `bs-o-cm-1-main-story-3` | Beastshield Main Story Vol.3: White Capital | `main` `final-battle` `twelve-divine-weapons` | [Index](chaptered-stories/main/bs-o-cm-1-main-story-3/README.md) |

### Side / 支线

| Code | Title | Tags | Link |
|------|-------|------|------|
| `bs-o-cs-1-yan-liang` | Yanliang | `side` `yanliang` | [Index](chaptered-stories/side/bs-o-cs-1-yan-liang/README.md) |
| `bs-o-cs-2-mo-quan` | Ink Hound | `side` `ink-hound` `martial-arts` | [Index](chaptered-stories/side/bs-o-cs-2-mo-quan/README.md) |
| `bs-o-cs-3-babysitter-lei` | Babysitter Lei | `side` `lao-lei` `martial-arts` `scorching-flow` | [Index](chaptered-stories/side/bs-o-cs-3-babysitter-lei/README.md) |

---

## III. Short Stories / 短篇作品

| Code | Title | Tags | Link |
|------|-------|------|------|
| `bs-o-s-1-farm-inn` | Beast Shield Farmhouse | `mystery` `dark` `farm` | [Read](short-stories/bs-o-s-1-farm-inn.md) |
| `bs-o-s-2-blackstone` | Blackstone | `action` `thriller` `bounty-hunter` | [Read](short-stories/bs-o-s-2-blackstone.md) |
| `bs-o-s-3-blackstone-2` | Blackstone 2 | `action` `thriller` `night-raid` | [Read](short-stories/bs-o-s-3-blackstone-2.md) |
| `bs-o-s-4-captive-delivery` | Captive Delivery | `psychological` `thriller` `captive` | [Read](short-stories/bs-o-s-4-captive-delivery.md) |
| `bs-o-s-5-employee-diary` | Beast Shield Employee's Day | `diary` `thriller` `survival` | [Read](short-stories/bs-o-s-5-employee-diary.md) |
| `bs-o-s-6-night-raid` | Night Raid | `thriller` `infiltration` `night-raid` | [Read](short-stories/bs-o-s-6-night-raid.md) |

---

## IV. Discovery / 发现入口

### Filter by Tags / 按标签筛选

- `main / 主线`: Beast Game World, Moying, White Capital
- `side / 支线`: Yanliang, Ink Hound, Babysitter Lei
- `bounty-hunter / 赏金猎人`: Blackstone, Blackstone 2
- `action / 动作`: Blackstone, Blackstone 2
- `thriller / 惊悚`: Blackstone, Blackstone 2, Captive Delivery, Beast Shield Employee's Day, Night Raid
- `psychological / 心理`: Captive Delivery
- `diary / 日记`: Beast Shield Employee's Day
- `farm / 农场`: Beast Shield Farmhouse
- `night-raid / 夜袭`: Blackstone 2, Night Raid

### By Beastkin Race / 按兽人种族

- Bovine / 牛兽人: Beast Shield Farmhouse
- Bear / 熊兽人: Beast Shield Employee's Day
- Tiger / 虎兽人: Beast Shield Employee's Day (Red Tiger)

---

## V. Language Versions / 语言版本

- [English Version / 英文版](./) (current / 当前页面)
- [Chinese Version / 中文版](../chinese/README.md)

---

[Return Up / 返回上级](../README.md)

*Last Updated / 最后更新: 2026-07-09*
