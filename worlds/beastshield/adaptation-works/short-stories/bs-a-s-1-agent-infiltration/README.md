# 潜入者之焰 / The Infiltrator's Flame

> 状态：completed · 编码：`bs-a-s-1-agent-infiltration` · 作者：memeticsingularity · 最后更新：2026-05-16

---

> **⚠️ 内容分级与警告 / Content Rating & Warnings**
>
> **成人内容 / 18+ ONLY**
>
> | 维度 Dimension | 级别 Level | 说明 Description |
> |:--|:--:|:--|
> | 🔞 性内容 / Sexual Content | **Lv.4** | 灼流暗劲转化为性快感，五只G级杂兵在控制与失控间被处决；含强制高潮、力量控制的性暗示、死后精液描写 |
> | 💀 暴力与死亡 / Violence & Death | **Lv.4** | 五场一对一仓库处决，年轻特工逐个击杀守卫；含掐颈、暗劲灌入、射精死亡等明确死法 |
> | 🧠 心理黑暗度 / Psychological Darkness | **Lv.3** | 年轻杀手对权力与控制的迷恋、"正义"的暧昧性、从毕业实习到独立任务的心理转变、猎物求饶与杀手戏谑 |
> | ⚡ 特殊触发 / Specific Triggers | — | `非自愿性快感（功法性）` `射精死亡` `力量控制/玩弄` `年轻杀手成长` `编号身份` `紧身制服物化` `死后亵玩（轻度）` |
>
> ⚠️ **世界观核心机制说明**：本作品所在世界观中，所有物理伤害会转化为目标的性欲值，蓄满后目标强制射精并死亡。灼流暗劲可加速并强化此过程。
>
> **请确保您已年满18周岁 / Please ensure you are at least 18 years old.**

---

## 一、简介 / Introduction

**一句话钩子**：年轻特工第一次独自执行任务，他本可以一拳一个，却选择让敌人在极致快感中缓慢凋零——因为总司令说，要学会控制。

特工局年轻特工**雷炎寰**奉命潜入兽盾偏远仓库。对这个掌握**灼流暗劲**的红虎兽人而言，这不仅是一次任务，更是一场关于力量、克制与成长的试炼。五只G级杂兵，五场死亡，五种控制的分寸——在精液与恐惧交织的仓库里，年轻的杀手第一次直面自己内心对权力的迷恋。

---

**One-line Hook**: A young agent with burning internal power is sent to clear five guards—and learns that the real test is controlling the beast inside himself.

Young agent Lei Yanhuan of the Secret Bureau is ordered to infiltrate a remote Beastshield warehouse. For this red tiger who wields the "Scorching Hidden Force," the mission is not merely about elimination, but about discipline, restraint, and the trial of power. Five G-rank guards, five deaths, five degrees of control—in a warehouse reeking of semen and fear, the young killer confronts his own fascination with dominance for the first time.

---

## 二、内容标签与核心元素 / Content Tags & Core Elements

### 🐾 物种 / Species
红虎兽人、野猪兽人、棕熊兽人、狗兽人、狼兽人

### 🔞 情色与身体 / Erotic & Bodily
`性欲值系统` `射精死亡` `灼流暗劲催情` `力量控制` `强制高潮` `紧身制服` `白袜/编号` `勃起描写` `精液描写`

### ⚔️ 死亡与暴力 / Death & Violence
`一对一击杀` `仓库处决` `掐颈` `暗劲灌入致死` `逐个猎杀` `赏金目标`

### 🏛️ 权力与关系 / Power & Relations
`年轻特工` `毕业实习后首次任务` `总司令教导` `力量克制` `戏谑与掌控` `猎物求饶`

### 🎭 主题与情感 / Themes & Emotions
`成长试炼` `正义的暧昧性` `权力迷恋` `控制与失控` `残酷诗意` `年轻气盛`

### 🎨 叙事与风格 / Narrative & Style
`第三人称限知视角` `动作细腻` `心理刻画` `九场景结构` `直白露骨`

### 🗺️ 核心场景 / Key Settings
`偏远仓库` `荒郊野岭` `仓库外围` `仓库内部`

### 🧩 世界观机制 / World Mechanics
`性欲值/Libido系统` `灼流暗劲` `特工局等级` `赏金系统` `制服编码`

### 🎯 结局指向 / Ending Tendency
`任务完成` `力量觉醒` `成长标志`

---

## II. Content Tags & Core Elements

### 🐾 Species
Red Tiger, Boar, Brown Bear, Dog, Wolf

### 🔞 Erotic & Bodily
`Libido System` `Ejaculation Death` `Scorching Hidden Force Arousal` `Power Control` `Forced Climax` `Tight Uniform` `White Socks/Number` `Erection Description` `Semen Description`

### ⚔️ Death & Violence
`One-on-one Kill` `Warehouse Execution` `Throat Crushing` `Dark Force Infusion Death` `Serial Hunt` `Bounty Target`

### 🏛️ Power & Relations
`Young Agent` `First Mission After Graduation` `Commander`s Teaching` `Power Restraint` `Teasing & Control` `Prey Pleading`

### 🎭 Themes & Emotions
`Growth Trial` `Ambiguous Justice` `Power Fascination` `Control & Loss of Control` `Cruel Poetry` `Young Arrogance`

### 🎨 Narrative & Style
`Third-person Limited` `Detailed Action` `Psychological Depth` `Nine-scene Structure` `Explicit`

### 🗺️ Key Settings
`Remote Warehouse` `Wilderness` `Warehouse Perimeter` `Warehouse Interior`

### 🧩 World Mechanics
`Libido System` `Scorching Hidden Force` `Agent Bureau Hierarchy` `Bounty System` `Uniform Code`

### 🎯 Ending Tendency
`Mission Complete` `Power Awakening` `Growth Milestone`

---

## 三、主要角色 / Characters

| 角色名 | 种族 | 身份/职业 | 简介（无剧透） |
|--------|------|----------|---------------|
| 雷炎寰 | 红虎兽人 | 特工局年轻特工 | 掌握灼流暗劲的实习生，正在学习控制力量的分寸 |
| G-533（猪大壮） | 野猪兽人 | 兽盾仓库守卫 | 只想安安分分守门、寄钱回家的底层杂兵 |
| G-534（熊厚实） | 棕熊兽人 | 兽盾仓库守卫 | 懒惰怕事的胖熊，被推去守夜却遭遇杀机 |
| G-536（狗来福） | 狗兽人 | 兽盾仓库守卫 | 胆小求饶的年轻守卫，在恐惧中达到高潮 |

---

## III. Characters

| Name | Species | Role | Brief Description |
|------|---------|------|-------------------|
| Lei Yanhuan | Red Tiger | Young Secret Bureau agent | A trainee who wields the Scorching Hidden Force, learning the measure of control |
| G-533 (Zhu Dazhuang) | Boar | Beastshield warehouse guard | A bottom-rank guard who just wants to send money home |
| G-534 (Xiong Houshi) | Brown Bear | Beastshield warehouse guard | A lazy, fearful bear who gets caught in the crossfire |
| G-536 (Gou Laifu) | Dog | Beastshield warehouse guard | A timid young guard who begs for his life |

---

## 四、章节列表 / Chapters

| 章节 | 标题 | 链接 |
|------|------|------|
| Ch.1 | 潜入者之焰（全一话·九场景） | [阅读](bs-a-s-1-agent-infiltration.md) |

---

## IV. Chapters

| Chapter | Title | Link |
|---------|-------|------|
| Ch.1 | The Infiltrator's Flame (One-shot, 9 Scenes) | [Read](bs-a-s-1-agent-infiltration.md) |

---

## 五、关联作品 / Related Works

- 原作/改编来源：`bs-o-cm-2-moying` — 墨影
- 同一世界观：`bs-a-s-2-vr-newbie`、`bs-a-cs-11-daily`

---

## V. Related Works

- Original / Source: `bs-o-cm-2-moying` — Mo Ying
- Same Universe: `bs-a-s-2-vr-newbie`, `bs-a-cs-11-daily`

---

## 六、创作留痕 / Creation Trail

- [查看变更日志](.process/CHANGELOG.md)
- [查看 AI 讨论索引](.process/ai-discussion/INDEX.md)

---

## VI. Creation Trail

- [View Changelog](.process/CHANGELOG.md)
- [View AI Discussion Index](.process/ai-discussion/INDEX.md)

---

*最后更新：2026-05-16 · Last updated: 2026-05-16*
