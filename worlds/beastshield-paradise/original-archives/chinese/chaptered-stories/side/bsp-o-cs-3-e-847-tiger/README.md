# 后巷的筹码：雷山 / Back Alley Bargain: Leishan

> **⚠️ 内容分级与警告 / Content Rating & Warnings**
>
> **成人内容 / 18+ ONLY**
>
> | 维度 Dimension | 级别 Level | 说明 Description |
> |:--|:--:|:--|
> | 🔞 性内容 / Sexual Content | **Lv.3** | 明确的身体弱点暴露、下体/护裆描写、制服 fetish、项圈控制与催情机制，但核心服务于忠诚与信任的叙事 |
> | 💀 暴力与死亡 / Violence & Death | **Lv.2** | 后巷尸体、赏金猎杀、兽盾清理队威胁、自律项圈处决机制，但无主要角色死亡 |
> | 🧠 心理黑暗度 / Psychological Darkness | **Lv.3** | 自愿臣服的心理动机、身份剥离（E-847→雷山）、工具化与去工具化的挣扎、对"被需要"的深层渴望 |
> | ⚡ 特殊触发 / Specific Triggers | — | 自愿戴项圈/枷锁、弱点暴露、催情电击处决、编号身份剥离、债务压迫背景、父母双亡 |
>
> ⚠️ **世界观核心机制说明**：本世界继承《兽盾》基础设定——精槽系统（伤害转化为性欲值，满槽射精死亡）。核心机制为"随从契约"——通过威逼利诱、实力征服或自愿臣服使NPC成为随从；本故事展现的是自愿臣服路径，以"自律项圈"作为忠诚的物化象征。
>
> **请确保您已年满18周岁 / Please ensure you are at least 18 years old.**
> 如不适应当前内容，请立即停止阅读 / If the content makes you uncomfortable, please stop reading immediately.

---

## 基本信息 / Basic Information

- **编码 / Code**: `bsp-o-cs-3-e-847-tiger`
- **中文标题 / Chinese Title**: 后巷的筹码：雷山
- **英文标题 / English Title**: Back Alley Bargain: Leishan
- **格式 / Format**: 分章故事 / Chaptered Story
- **子类型 / Subtype**: 支线 / Side
- **状态 / Status**: 已完成 / Completed
- **位置 / Location**: original-archives
- **作者 / Author**: memeticsingularity
- **世界观 / Universe**: Beastshield Paradise

---

## 故事简介 / Synopsis

凌晨三点，白之都的后巷弥漫着垃圾腐败的酸臭。赏金猎人李游刚处理完一个落单的G级野猪兽人，一抬头，却看见巷口站着一个高大的黑影——E级武斗兵，虎兽人雷山。但出乎意料的是，对方并没有发起攻击，而是缓缓举起双手，提出了一笔交易：他愿意付出三个筹码——情报、弱点、忠诚——来成为李游的随从。

At three in the morning, the back alleys of White Capital reek of rotting garbage. Bounty hunter Li You has just finished off a lone G-level wild boar beastkin when he looks up to see a tall black figure at the alley entrance—E-level combat soldier, tiger beastkin Leishan. But unexpectedly, the other party doesn't attack. Instead, he slowly raises his hands and proposes a deal: he is willing to offer three chips—intelligence, weakness, and loyalty—to become Li You's follower.

---

## 内容标签与核心元素 / Content Tags & Core Elements

### 🐾 物种 / Species
虎兽人、野猪兽人、黑牛兽人（提及）

### 🔞 情色与身体 / Erotic & Bodily
`精槽机制` `弱点暴露` `护裆/金属护具` `制服 fetish` `黑制服` `长筒袜` `自律项圈` `催情电击` `下体描写` `股动脉薄弱点`

### ⚔️ 死亡与暴力 / Death & Violence
`赏金猎杀` `后巷尸体` `兽盾清理队` `自律项圈处决` `飞刀` `匕首`

### 🏛️ 权力与关系 / Power & Relations
`玩家-随从` `自愿臣服` `赏金猎人-武斗兵` `上级-下属` `兽盾等级体系（E级/G级）` `主从契约`

### 🎭 主题与情感 / Themes & Emotions
`自愿的臣服` `筹码与信任` `身份重塑` `被需要的渴望` `选择被谁束缚` `救赎与归属`

### 🎨 叙事与风格 / Narrative & Style
`交易叙事` `后巷 noir` `权力反转` `身体政治` `忠诚考验`

### 🗺️ 核心场景 / Key Settings
`白之都后巷` `巷口路灯` `兽盾总部通风管道` `黑市`

### 🧩 世界观机制 / World Mechanics
`精槽系统` `射精死亡` `随从契约` `自律项圈` `兽盾编号体系` `赏金系统` `生物密钥绑定`

### 🎯 结局指向 / Ending Tendency
`随从结局` `开放式冒险` `主从羁绊`

---

## 章节列表 / Chapter List

- [第一章：后巷的筹码 / Chapter 1: Back Alley Bargain](ch-1.md)
- [第二章：筹码的代价 / Chapter 2: Price of the Bargain](ch-2-1.md)
- [第三章：忠诚的试炼 / Chapter 3: Trial of Loyalty](ch-3-1.md)
- [第四章：新的羁绊 / Chapter 4: New Bonds](ch-4-1.md)

---

## 核心主题 / Core Themes

- **自愿的臣服**: 在残酷世界中，选择被谁束缚已是最大的自由
- **筹码与信任**: 情报、弱点、忠诚——层层递进的信任建立
- **身份的重塑**: 从兽盾编号E-847到独立的"雷山"

---

## 语言版本 / Language Versions

- [中文版 / Chinese Version](./) (当前页面 / current)
- [英文版 / English Version](../../../../english/chaptered-stories/side/bsp-o-cs-3-e-847-tiger/README.md)

---

[返回支线目录 / Return to Side Stories](../README.md) | [返回兽盾乐园首页 / Return to Beastshield Paradise Home](../../../../../README.md)
