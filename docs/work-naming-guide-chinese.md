...
本文档规定了 Beastkin Universe 项目中作品的命名规则和组织结构。所有贡献者创作新作品时，请遵循本指南。本指南根据最新项目结构更新，确保所有作品类型命名格式统一，采用自然数编号，并统一了所有分章故事的结构。
...
存放由社区创作者基于官方作品进行的 **改编创作**。这些作品可视为官方世界的衍生演绎。改编作品在完成且获得认可后，可申请 **晋升
**。
...
A: 为了在编码中明确区分作品类型，使文件编码自包含所有关键信息。看到编码就知道是主线分章(cm)、支线分章(cs)还是短篇(s)
，无需查看目录结构。
...
A: 是的，改编作品同样遵循这个规则。如果改编的是主线内容，使用`cm`；如果是支线内容，使用`cs`。这有助于理解作品的定位和内容性质。
...
A: 可以。当作品完成并达到一定质量标准后，可以申请迁移到 `original-archives`。迁移时会根据需要重命名文件，并调整到正确的目录（main或side）。
